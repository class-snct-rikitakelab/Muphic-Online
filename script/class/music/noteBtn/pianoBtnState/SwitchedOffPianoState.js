var SwitchedOffPianoState = enchant.Class.create(State, {
	// コンストラクタ
	...
	_touchEndBehavior : function(otherButton) {
		...
		this._parent._setState(new SwitchedOnPianoState(this._parent));
		this._parent._setSelectPianoState();
		otherButton._setIsPush(false);
		otherButton._setState(new NPushRemoveState(otherButton));
	},
	...
	_frameBehavior : function() {
		...
		if(mOverX === true && mOverY === true) {
			...
			this._parent._setState(new NPushPianoState(this._parent));
		}
	},
})