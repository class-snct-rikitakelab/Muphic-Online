var LadyPropertyButton = enchant.Class.create(StorySceneSprite, {
	...
	_setOnOffImage : function(state) {
		if(state === "on") {
			this.image = core.assets[STORY_PALETTEPROPERTY_LADY_ON._path];
		} else {
			this.image = core.assets[STORY_PALETTEPROPERTY_LADY_OFF._path];
		}
	},
	// 押下, 非押下時の画像切り替え
	_setPushImage : function(state) {
		if(state === "push") {
			this.image = core.assets[STORY_PALETTEPROPERTY_LADY_PUSH._path];
		} else {
			this.image = core.assets[STORY_PALETTEPROPERTY_LADY_OFF._path];
		}
	},
	...
	ontouchend : function(event) {
		...
		if(myselfPush === false && otherPush !== null) {
			...
			this._parent._setPushButton("lady");
			this._parent._requestAddIllust("lady");
		}
	},
