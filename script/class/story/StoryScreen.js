...
var StoryScreen = enchant.Class.create({
	...
	_createSaveButton : function() {
		var path = STORY_SAVEBUTTON_OFF._path;
		var width = STORY_SAVEBUTTON_OFF._width;
		var height = STORY_SAVEBUTTON_OFF._height;
		var x = 700;
		var y = 100;
		this._saveButton = new StorySaveButton(path, width, height, x, y, this);
	},
	// 読み込みボタンのオブジェクトを生成
	_createLoadButton : function() {
		var path = STORY_LOADBUTTON_OFF._path;
		var width = STORY_LOADBUTTON_OFF._width;
		var height = STORY_LOADBUTTON_OFF._height;
		var x = 560;
		var y = 100;
		this._loadButton = new StoryLoadButton(path, width, height, x, y, this);
	},
	// プレビュー画面のオブジェクトを生成
	...
	_setCanPushSaveButton : function() {
		this._saveButton._setTouchEnabled(true);
		this._saveButton._setDarkImage(false);
	},
	// 保存ボタンを押せない状態に
	_setCannotPushSaveButton : function() {
		this._saveButton._setTouchEnabled(false);
		this._saveButton._setDarkImage(true);
	},
	// 読み込みボタンを押せる状態に
	_setCanPushLoadButton : function() {
		this._loadButton._setTouchEnabled(true);
		this._loadButton._setDarkImage(false);
	},
	// 読み込みボタンを押せない状態に
	_setCannotPushLoadButton : function() {
		this._loadButton._setTouchEnabled(false);
		this._loadButton._setDarkImage(true);
	},
	// イラストボタン全体を押せる状態に
